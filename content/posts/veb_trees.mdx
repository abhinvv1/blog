...
description: "Van Emde Boas tree provides O(log log U) TC for insert, delete, search, successor, and predecessor operations on integer sets."
date: "2025-10-03"
...
```
if x == min or x == max → found
else look into cluster[high(x)] for low(x)
```
