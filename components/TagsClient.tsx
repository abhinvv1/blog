...
interface TagsClientProps {
  posts: Post[];
  projects: Project[];
  postTags: { tag: string; count: number }[];
  projectTags: { tag: string; count: number }[];
}
...
export function TagsClient({
  ...
  const [contentType, setContentType] = useState<"all" | "posts" | "projects">(
    "all"
  );
  ...
  const allTags = React.useMemo(() => {
    const combined = new Map<
      string,
      { tag: string; postCount: number; projectCount: number }
    >();
    ...
    return Array.from(combined.values()).sort(
      (a, b) => b.postCount + b.projectCount - (a.postCount + a.projectCount)
    );
    ...
    <div className="max-w-4xl">
      ...
      <div className="mb-12">
        ...
        <p className="text-dark-300">
          Browse content by tags. Click on any tag to see related posts and
          projects.
        </p>
      </div>
              ...
              <button
                ...
                className={`px-3 py-1 text-sm rounded-md transition-colors duration-200 ${
                  contentType === "all"
                    ...
                    : "bg-gray-700 text-white hover:bg-gray-600 dark:bg-dark-800 dark:text-white dark:hover:bg-dark-700"
                }`}
              >
                    ...
                    >
                      <div className="space-y-3">
                        <div className="flex items-start justify-between">
                          ...
                          <span
                            className={`text-xs px-2 py-1 rounded-md ml-4 ${
          ...
          {((contentType === "posts" && filteredPosts.length === 0) ||
            ...
            <div className="text-center py-12">
              ...
              <p className="text-dark-400 text-lg">
                No {contentType === "all" ? "content" : contentType} found for
                tag "{activeTag}"
              </p>
              <button
                onClick={clearTag}
